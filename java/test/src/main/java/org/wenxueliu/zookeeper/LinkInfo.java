...
public class LinkInfo {
	...
	public LinkInfo(Date firstSeenTime, Date lastLldpReceivedTime, Date lastBddpReceivedTime) {
		...
		this.latencyHistory = new ArrayDeque<U64>(100);
		this.latencyHistoryWindow = LATENCY_HISTORY_SIZE;
		this.latencyUpdateThreshold = LATENCY_UPDATE_THRESHOLD;
	}
    ...
    //    return Bytes.concat(
    //            this.src.getBytes(),
    //            Ints.toByteArray(this.srcPort.getPortNumber()),
    //            this.dst.getBytes(),
    //            Ints.toByteArray(this.dstPort.getPortNumber()),
    //            Longs.toByteArray(this.latency.getValue())
    //        );
    //}

    // @Override
    //public LinkInfo readFrom(byte[] bytes) {
    //    System.out.println("decode");
    //    try (ByteArrayInputStream bis = new ByteArrayInputStream(bytes);
    //        ObjectInput in = new ObjectInputStream(bis)) {
    //        return (LinkInfo)in.readObject();
    //    } catch(IOException e) {
